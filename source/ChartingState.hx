...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function update(elapsed:Float)
	{
		...
		if (_song.noteStyle == 'eightnotes' && gridBG.width != GRID_SIZE * 16)
		{
			remove(gridBG);
			...
			&& FlxG.mouse.y < gridBG.y + (GRID_SIZE * _song.notes[curSection].lengthInSteps))
		{
			...
			dummyArrow.x = arX;
			if (_song.noteStyle == 'eightnotes') arX = Math.floor(FlxG.mouse.x / GRID_SIZE) * GRID_SIZE;
						dummyArrow.x = arX;
	...
	function updateGrid():Void
	{
		...
		for (i in sectionInfo)
		{
			...
			note.updateHitbox();
			note.jumpscare = daNoteInfo > 7 && daNoteInfo < 15; 
			note.intensenote = daNoteInfo > 15;
			if(note.intensenote){note.jumpscare = false;}
	...
	private function addNote(?n:Note):Void
	{
		...
		var noteData = Math.floor(FlxG.mouse.x / GRID_SIZE) + (FlxG.keys.pressed.ALT ? 8 : 0)  + (FlxG.keys.pressed.CONTROL ? 16 : 0);
		var noteSus = 0;
		if (_song.noteStyle == 'eightnotes')
		{
